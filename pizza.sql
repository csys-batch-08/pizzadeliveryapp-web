...
select users.user_name ,sum(orders.total_prize) total_prize from orders join users on users.user_id=orders.user_id where orders.order_date between '05-01-22' and '13-01-22' group by users.user_name  order by total_prize desc;